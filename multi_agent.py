...
def web_search(query:str): 
    """Search the web for information.Useful for searching latest information on the internet.Use this when you need to find current or factual information.""" 
...
def web_agent_tool(query: str):
    ...
    response = create_web_agent.invoke({"messages": [{"role": "user", "content": query}]})
...
def poem_agent_tool(query: str):
    ...
    poem_result = create_poem_agent.invoke({"messages": [{"role": "user", "content": query}]})
    return poem_result.get('messages')[-1].content
...
git_tools = composio.tools.get( 
    ...
    tools=[
        ...
        "GITHUB_GET_A_REPOSITORY","GITHUB_RENAME_A_BRANCH","GITHUB_COMMIT_MULTIPLE_FILES","GITHUB_GET_A_BRANCH"]) 
...
gmail_tools = composio.tools.get( 
    user_id=USER_ID,
    tools=["GMAIL_FETCH_EMAILS","GMAIL_SEND_EMAIL","GMAIL_FORWARD_MESSAGE","GMAIL_CREATE_EMAIL_DRAFT","GMAIL_LIST_DRAFTS"] ) 
...
create_gmail_agent = create_agent( 
    ...
    - When sending an email, always use the provided sender name in the "Best Regards" section; if not provided, use the sender name, and never guess the sender names.
""" 
) 
...
4. Do not mention which tool you used.
5. Be smart and chain agents when required.
"""
...
def call_agent(query: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"error in call-agent{e}")